...
export const getOidcCredential = async (
  ...
  try {
    const url = `${getRuntimeEnv("BLOCKS_MONITOR_BASE_URL")}${AUTH_OIDC_ENDPOINTS.GET_OIDC_CLIENT}?ProjectKey=${payload.projectKey}&ClientId=${payload.clientId}`;
...
export const userAcknowledgement = async (
  ...
  try {
    const url = `${getRuntimeEnv("BLOCKS_MONITOR_BASE_URL")}${OIDC_FLOW_ENDPOINTS.USER_ACKNOWLEDGEMENT}`;
...
export const accountRecover = async (
  ...
  try {
    const url = `${getRuntimeEnv("BLOCKS_MONITOR_BASE_URL")}${ACCOUNT_ENDPOINTS.RECOVER}`;
