...
/** Blocks console on Monitor or OS — re-authenticates when the session expired. */
...
/**
 * Creates a project via the OS-hosted wizard (Monitor redirects to OS for this).
 *
 * Flow:
 *   Monitor console → "Add Project" → redirected to OS create-project wizard →
 *   fill wizard → project created on OS (environments or console) →
 *   navigate to Monitor console → open the new project → Monitor dashboard
 */
