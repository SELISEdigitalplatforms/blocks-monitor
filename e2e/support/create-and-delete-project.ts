...
const HOME_APP_NAME = /Monitor/i
const HOME_APP_URL = /dev-monitor|localhost/
...
export async function createProject(page: Page) {
  await test.step("Start a new project (redirects to OS)", async () => {
    ...
    if (await loginButton.isVisible().catch(() => false)) {
      ...
      if (!await nameHeading.isVisible().catch(() => false)) {
        const { e2eBaseUrl: getBase } = await import("./env")
        await page.goto(`${getBase()}/app/console`, { waitUntil: "domcontentloaded" })
        await ensureConsole(page)
  ...
  await test.step("Switch back to Monitor via app switcher", async () => {
    await clickAppSwitcherAndNavigate(page, HOME_APP_NAME)

    await page.waitForURL(HOME_APP_URL, { timeout: 60_000 }).catch(async () => {
      const base = (await import("./env")).e2eBaseUrl()
  ...
  await test.step("Open the newly created project on Monitor", async () => {
...
export async function deleteCreatedProject(page: Page, projectName?: string) {
  ...
  await test.step("Return to Monitor and verify e2e projects are gone", async () => {
    await clickAppSwitcherAndNavigate(page, HOME_APP_NAME)
    await page.waitForURL(HOME_APP_URL, { timeout: 60_000 }).catch(async () => {
      const base = (await import("./env")).e2eBaseUrl()
      await page.goto(`${base}/app/console`, { waitUntil: "domcontentloaded" })
