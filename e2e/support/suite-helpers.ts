...
export async function openSharedProjectDashboard(page: Page) {
  ...
  if (!fixture?.itemId) {
    throw new Error(
      "Missing fixtures/monitor-project.json (or itemId) — run the monitor-setup project first " +
        "(suite.setup.spec.ts).",
    )
  }
  ...
  try {
    await waitForMonitorDashboardReady(page, fixture.projectName)
    await persistSuiteSession(page)
  } catch (error) {
